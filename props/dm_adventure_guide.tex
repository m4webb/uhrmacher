...
\midrule
\hyperref[sec:clock]{Session Clock} & Pacing & \pageref{sec:clock} \\
...
\hyperref[sec:epilogue]{7. Epilogue} & Aftermath --- Beats & \pageref{sec:epilogue} \\
\midrule
...
\vspace{6pt}
{\large\bfseries\scshape Conformity Escalation}\\[-2pt]
\textcolor{rulecol}{\rule{\textwidth}{0.6pt}}
\textit{Apply as the game progresses; jump a full beat when the Gear is installed. All flavor, no mechanics.}\\[2pt]
\renewcommand{\arraystretch}{1.05}
\begin{tabularx}{\textwidth}{@{}clX@{}}
\toprule
\textbf{Beat} & \textbf{Sense} & \textbf{Effect} \\
\midrule
1 & Sight & Colors begin to desaturate --- flowers look duller, wood grain fades \\
2 & Sound & Ambient sounds fall into rhythm --- birdsong syncs to the clock's tick \\
...
5 & Environment & Vegetation grows unnaturally straight --- wildflowers in rows, branches at right angles \\
6 & Environment & Architecture straightens --- crooked timbers become plumb, cobblestones align to grids \\
...
\phantomsection\label{sec:clock}\sceneheader{Session Clock}{4:00 play}{Pacing}{Act III at three o'clock}
...
\toprule
\textbf{Clock} & \textbf{Budget} & \textbf{Segment} \\
\midrule
0:00 & 35 min & Opening read-aloud + the Gasthaus (all of Act I) \\
0:35 & 25 min & Forest approach + Clockhall, Pendel \\
1:00 & --- & \textbf{Checkpoint: inside the tower, job in hand} \\
1:00 & 40 min & Bellows Chamber (first combat --- 7 players finding the rhythm) \\
1:40 & 25 min & Break ({\texttildelow}10 min) + second Clockhall pass; Study / Linde / contract \\
2:05 & 40 min & Winding Room: Fritz, the golem, the maze \\
2:45 & 15 min & \textbf{Checkpoint: Gear installed} --- the snap --- the climb \\
3:00 & 45 min & \textbf{Act III: the tower strikes three} --- hour three at the table is three o'clock in Freudenfeld \\
3:45 & 15 min & Epilogue --- play the Fritz beat; summarize the rest \\
\bottomrule
\end{tabularx}

\columnbreak

\section*{Catch-Up Levers}
\begin{itemize}
\item \textbf{Behind at 1:00:} trim Heinrich to his key line; Grüber gives directions without the back room
\item \textbf{Behind at 2:05:} use the bellows dial (a swarm at half HP flees up the pipes); narrate the second Clockhall pass; Linde hands over the contract after one good beat instead of extended trust-building
\item \textbf{Behind at 2:45:} fast-lane the social path (Fritz spotted early; Persuasion 12 lands); the maze teaches its lesson in two pulls instead of a full exploration
\item \textbf{Hard floor at 3:15:} start Act III regardless --- a rushed ending beats no ending
\end{itemize}

\section*{Never Cut}
Fritz, the snap, and the epilogue's Fritz beat.

\section*{Ahead of Schedule}
Let Pendel's arc breathe; more maze play; Heinrich gets a second story.
...
\section*{Read-Aloud --- Session Opening}
\textit{Hand out the Bekanntmachung (pest-control ad) first; read this once they've seen it.}
\begin{readaloud}
\itshape
In the high country far from the cities, where the maps go vague and the forests go old, you have traveled together long enough to know each other's snoring --- a company with its own reasons, welded by the road: through villages of dark timber and white plaster, past wayside shrines with their candle stubs, over bridges barely wider than the carts that cross them. It is handsome country and poor employment; folk here solve their own problems, or learn to live alongside them. So when a printed notice in a market square offered twenty-five gold pieces a head --- pest control, the township of Freudenfeld --- no one argued long.

Freudenfeld lies at the end of its own road, beneath the eaves of the Tiefenwald: the Deep Forest, which the locals name the way one names a neighbor --- with respect, and no great wish to visit. You come down to it in the late afternoon, and it is not the town you expected. It is doing well. Fences mended, hedges square, fat geese in the lanes, the harvest stacked in ricks so neat they look drawn with a rule. Children watch you pass and do not follow. And ahead, where the street gathers itself around a well, there is lamplight and the smell of bread and beer: the Gasthaus, where the Bürgermeister is expecting you.
\end{readaloud}

\section*{Read-Aloud --- The Gasthaus}
\begin{readaloud}
\itshape
...
\subsection*{First Impression (at his table)}
\begin{readaloud}
\itshape
...
\begin{itemize}
\item Saw a Tickwichtel in the woods as a boy of seven --- it barely noticed him, but the memory shaped his life
...
\subsection*{When Fritz Is Found (Perception DC 12)}
\begin{readaloud}
\itshape
...
\subsection*{When the Golem Is Released}
\begin{readaloud}
\itshape
...
\item \textbf{Six colossal cast-iron pinecone counterweights} (3$\times$3 each), one above each berth, at two levels (rows E and I). Exactly two per level are down at any moment --- \textbf{four on the map, never more}; each level always has exactly one lane open. A pull transfers the load: one weight descends as another rises.
...
\begin{tabularx}{\columnwidth}{@{}llX@{}}
\toprule
\textbf{Chain} & \textbf{Weights} & \textbf{Effect} \\
\midrule
G2 & $\downarrow$I2\,$\uparrow$I5 & opens lower mid --- \textbf{first effective pull: traps the puller, releases the golem} \\
G6 & I5$\downarrow$\,$\uparrow$I2 & the re-arm chain, beside the golem's cage \\
M3 & $\downarrow$I5\,$\uparrow$I8 & opens lower east \\
K2 & I8$\downarrow$\,$\uparrow$I5 & opens lower middle \\
B4 & E5$\downarrow$\,$\uparrow$E2 & backroom endgame control \\
C3 & $\downarrow$E2\,$\uparrow$E5 & backroom endgame control \\
L2 & $\downarrow$E5\,$\uparrow$E8 & opens upper east \\
C8 & E8$\downarrow$\,$\uparrow$E5 & opens upper middle, \textbf{seals the puller in Fritz's corner} --- Fritz can pull it himself: the social path's payoff \\
\bottomrule
\end{tabularx}

\vspace{4pt}
Start: weights down at E2, E5, I5, I8 --- upper open east, lower open west, no route through. Minimum two pulls to any route (G2 $\to$ C8). The ``west parade'' victory exists: both levels open west, party walks past the caged golem. Full layout: \texttt{scenes/encounter-04-winding-room.md} + DM guide map.
...
The full Contract of Planar Accord (physical prop --- \texttt{props/documents/contract.tex}). Linde has it and knows it matters, but can't read it. Players must earn her trust to get it. Art.\ VI \S6.3 is the key: a failed activation sequence nullifies the contract entirely. Art.\ IV supplies the timing: the sequence commences at the striking of three --- the party can plan their ascent around it.
...
\section*{Victory}
Cuckoo destroyed before completing 3 cycles. Contract null and void (Art.\ VI \S6.3). Aperture closes; the tower stops ticking --- the Gear has cracked cleanly in two. The modrons on this plane gather to Pendel. The broken cuckoo remains.
...
\section*{Read-Aloud --- First Emergence}
\begin{readaloud}
\itshape
...
\end{readaloud}
\textit{Pause after the slam for the door-slam DEX saves (and knockdown); the last line is the Call --- roll the CON saves there. The party should reach the landing just before three --- the sequence commences on the hour (Art.\ IV); each Call is one stroke.}
...
\textbf{Construct:} Immune to poison and psychic damage; immune to the poisoned, charmed, frightened, exhaustion, paralyzed, and petrified conditions. No damage resistances --- its HP and the battlefield are its defense.\\
...
\midrule
1 (ON) & Rotate. Doors slam (swing tiles). Emerges --- knockdown (DC 14 DEX). Wave 1. The Call --- first stroke of three. \\
2 (OFF) & Rotate. Retracts --- knockdown. Hold-on window (DC 12 STR to ride in). Doors close. \\
3 (ON) & Rotate. Doors slam. Emerges --- knockdown. Wave 2. The Call --- second stroke. \\
4 (OFF) & Rotate. Retracts --- knockdown. Hold-on window. Doors close. \\
5 (ON) & Rotate. Doors slam. Emerges --- knockdown. Wave 3. The Call --- third stroke completes the anchor. Last chance. \\
\bottomrule
\end{tabularx}
...
\item \textbf{Table procedure:} the platform sheet (doors closed) stays over row A all fight; the gear map rotates beneath it. ON cycles: set the rod piece on top --- its head is the open doorway, its perch lands on the hub (the cuckoo is Large: 2$\times$2 on the hub intersection). OFF: lift it off.
\end{itemize}
...
Primary: the contract --- Art.\ VI \S6.3 is the termination clause, Art.\ IV the timing (commencement at the striking of three, each cycle one stroke). Backup (DM's choice): Pendel states it (``Activation at three o'clock. Precisely.''), Linde spells it out, or a modron announces ``second emergence logged; one remains.'' Deliver it by the end of cycle 1 if nobody has connected it.
...
\section*{End Conditions}
\textbf{Victory:} cuckoo at 0 HP before 3 full cycles. Contract null and void (\S6.3). Aperture closes; the tower stops ticking (the Gear has cracked cleanly in two). The modrons stand down and gather to Pendel. See the Epilogue page.\\
\textbf{Defeat:} the tower finishes striking three --- 3 cycles complete. Planar anchor established. Conformity permanent.\\
...
\subsection*{Coven key stats (2014 SRD --- spot-check vs.\ your MM)}
\textbf{Green Hag (CR 3):} AC 17, HP 82, 30 ft; Claws +6, 13 (2d8+4) slashing; Illusory Appearance, Invisible Passage, Mimicry. \textbf{Coven} (all three alive, within 30 ft): shared slots --- hold person, counterspell, lightning bolt, bestow curse.\\
\textbf{Minions:} Wolf --- AC 13, HP 11, Bite +4, 7 (2d4+2), DC 11 STR or prone, Pack Tactics. Needle Blight --- AC 12, HP 11, Needles +3, 8 (2d6+1), 60 ft. Twig Blight --- AC 13, HP 4, Claws +3, 3 (1d4+1).
...
\phantomsection\label{sec:epilogue}\sceneheader{Epilogue --- Beats}{Act III}{Aftermath}{No script --- run what the table earned}

\begin{multicols}{2}

\section*{Victory}
\begin{enumerate}
\item \textbf{The tower stops.} The first true silence --- no tick anywhere. Players holding the journal may recognize Wilhelm's line: \textit{``The silence was immediate and complete. I had not realized how loud it was until it was gone.''}
\item \textbf{The Gear.} If anyone investigates why the ticking stopped: in the winding room, the Gear has \textbf{cracked cleanly in two} in its cradle. The contract is void, and the crystallized law knew it.
\item \textbf{The modrons go to Pendel.} Those on this plane stand down and gather to the highest-ranking modron present. What happens next depends entirely on what happened to Pendel --- befriended/rogue, frozen neutral, or antagonized. Play it out; there is no script for a rogue modron with a congregation.
\item \textbf{Fritz.} With the ticking gone, his leg stops working. Where he is depends on Act II: befriended, he watched from the landing hatch; frightened or fought, he's below with the golem. Don't shortcut it --- this is the cost the party chose. Let it land.
\item \textbf{Linde.} The tower is dead; she got what she came for. The forest recovers as it did after 1742 --- mostly. The straight stretch of river stays straight.
\item \textbf{Freudenfeld.} Grüber pays 25gp a head, delighted, understanding nothing. The conformity recedes beat by beat (run the escalation table backward). Eichenwald's classroom gets noisy again.
\end{enumerate}
...
\section*{Defeat}
\begin{itemize}
\item The planar anchor is permanent. The blessings of Order flow --- roughly five leagues at full saturation per the contract's fine print, a radius ``revised upward without notice.''
\item The modrons no longer need exterminators. The town thanks the party warmly, in increasingly similar voices.
\end{itemize}

\section*{Alternate Path (party defended the cuckoo)}
\begin{itemize}
\item Same permanence, but chosen. The contract is fulfilled; the party is paid and remembered fondly by the Collective --- a line item in a ledger on Mechanus.
\item Linde's coven survives or doesn't; either way the Tiefenwald's wildness is on a clock.
\end{itemize}

\section*{DM Notes}
\begin{itemize}
\item The epilogue is where the Fritz dilemma pays off --- everything else can be summarized, but the Fritz beat should be played.
\item Pendel's fate is deliberately unresolved in his sheet; the epilogue is where the table resolves it.
\end{itemize}
...
\section*{Identity}
Male, 60s. Inherited the Bürgermeister's seat from his father. Landed gentry --- his family's wealth comes from owning land and collecting rents, not from trade or craft. He was a modestly successful businessman before taking over, but has never had to struggle for anything. Has a wife and several children.
...
When Heinrich was seven, around the time the Uhrmacher first built the tower, he saw a Tickwichtel in the woods. A small clicking creature that barely noticed him. Nothing actually happened --- it didn't interact with him, just went about its business --- but it was a deeply formative experience for a young boy. As the tower's effects escalated and then came to a mysterious end, he knew the Tickwichtel was somehow involved.
...
\textbf{Wächter} \textit{(VEKH-ter)} --- ``Watchman.'' The gear golem --- what Fritz's protector is, to Fritz. Used as its token label so the word ``golem'' never reaches the table.
...
\textbf{Stimmgabel} \textit{(SHTIM-gah-bel)} --- ``Tuning fork.''

\textbf{Uhrmacheröl} \textit{(OOR-mahkh-er-erl)} --- ``Clockmaker's oil.''

\textbf{Küken} \textit{(KOO-ken)} --- ``Chick.''

\textbf{Sprungfeder} \textit{(SHPROONG-fay-der)} --- ``Mainspring.''

\textbf{Moosbund} \textit{(MOHS-boont)} --- ``Moss-knot.''

\textbf{Minutenzeiger} \textit{(mi-NOO-ten-tsy-ger)} --- ``Minute hand.''

\textbf{Feinwerkzeug} \textit{(FINE-verk-tsoyk)} --- ``Fine tools.''

\textbf{Kirschwasser} \textit{(KEERSH-vahs-ser)} --- Black Forest cherry brandy.

\textbf{Schnitzfiguren} \textit{(SHNITS-fi-goo-ren)} --- ``Carved figures.''

\textbf{Doppelpfeife} \textit{(DOP-el-pfy-feh)} --- ``Double whistle.''

\textbf{Messingfedern} \textit{(MESS-ing-fay-dern)} --- ``Brass feathers.''
...
\textbf{Activation Sequence} --- The process of establishing the planar anchor. Three cuckoo cycles, commencing at the striking of three (Art.\ IV) --- each Call is one stroke of the hour. Must be started \textit{then stopped} to trigger \S6.3.
